...
**`.mcp.json`**:
The gitignored MCP server configuration at the repo root, generated by `/setup` from a
template shipped in the plugin. Holds literal credential values, never `${VAR}` — the
Windows Desktop app does not expand it. See `docs/adr/0004`.

**`outputs/`**:
The gitignored workspace at the repo root for generated planning artifacts — the domain
glossary, decision records/ADRs, and build specs the advisory skills produce. `/setup`
creates it; `outputs/README.md` is the only file tracked inside it. See `docs/adr/0005`.
