...
Issues and PRDs for this repo live in a **Jira board** — project key **`IV`**. Drive the
...
> **DEFERRED (2026-07-17).** The project-specific workstream label set for "Ignyte Appian Developer
> Copilot" has not been defined yet. Until it is, `to-tickets` should apply only the triage labels
> (below) and skip the workstream axis. Fill this in — replacing the example — before relying on
...
children's. Filter by the axis with JQL, e.g. `project = IV AND labels = <workstream-a>`.
...
- **Create an issue** — the MCP `create issue` tool: project `IV`, issuetype `Task`,
...
- **List / query** — the MCP `search (JQL)` tool, e.g.
  `project = IV AND status = "To Do" AND labels = ready-for-agent ORDER BY key`.
...
- **Claim** — assign the ticket to the driving user before any work (an open, unassigned child is
  unclaimed).
- **Frontier query** — `project = IV AND parent = <MAP_KEY> AND statusCategory != Done AND
  assignee IS EMPTY` (via the MCP search tool); unblocked children are those with no open blockers.

## When a skill says "publish to the issue tracker"

Create a `Task` in project `IV` via the Jira MCP, parented to the relevant feature Epic
...
Use the Jira MCP `get issue` tool for `IV-<n>` (the user will normally pass the key).
