...
**Authored in IADC-Advisor.** IADC-Tester carries a mirror of this module. The two copies differ
in exactly three places -- SKILLS_ROOT, this paragraph, and the file named in the go-red recipe
below -- and each carries its own baseline. Change this copy first, then bring it to the mirror.

To watch the ratchet go red: add the text `git status --short`, backticks included, anywhere in
iadc-advisor/skills/orient/SKILL.md and run `python3 -m pytest tests -v`. The failure names the
file, the baseline and the new count. Remove it and the suite is green again.
