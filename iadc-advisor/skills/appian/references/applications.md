...
- Process models → `process-models.md` (for per-node-type specifics, look the node up in the Appian documentation — `/iadc-advisor:context7` first, then version-exact `docs.appian.com` via the **Appian Documentation Search** workflow in `SKILL.md`; never guess a node's configuration)
