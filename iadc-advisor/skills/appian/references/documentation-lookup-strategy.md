...
**Fallback:** If the ambient Project configuration has no `Appian version` line (or it still holds an unfilled angle-bracket placeholder), default to 26.6 (latest) and tell the user you did.
...
- Version 26.5 redirects to 26.6 (see the Version Notes in `SKILL.md`)
...
```bash
# Fresh shell — substitute the Appian version from the ambient Project configuration.
VERSION="<Appian version>"
case "$VERSION" in *"<"*|"") echo "Substitute the configured Appian version into VERSION first." >&2; exit 1 ;; esac
