---
name: reconcile
description: "Close the loop on a ticket's escalated architectural gaps. After /iadc-advisor:pressure-test escalates a decision to the project lead (async, via Slack or a Jira comment), run this — with the ticket key — to check whether the lead has replied, fold their answers into the ticket's decisions, re-check impact on the approach, and update readiness. Use when the lead has responded to an escalation, or to check whether they have. Verbs: reconcile, check escalations, did the lead reply, pull in the lead's answer, gumby-reconcile, close the loop on TICKET."
argument-hint: "the ticket key to reconcile, e.g. IMM-2"
---

# Reconcile — close the escalation loop

`/iadc-advisor:pressure-test` escalates architectural gaps to the project lead **asynchronously** (Slack DM/channel, or
...
It's the same discipline as `/iadc-advisor:pressure-test` — read-mostly, gated writes, capture to the outputs workspace
...
2. **Check each thread for a reply.**
   ...
   - **Jira comment** — read the ticket's comments via `/iadc-advisor:jira` for the lead's response.
...
4. **Record the resolution.** A resolved escalation **is a substantive decision**, so capture it the
   same way `/iadc-advisor:pressure-test` captures decisions (via `/iadc-advisor:domain-modeling`): write its own **numbered
...
5. **Re-check impact.** Compare the answer to the provisional lean Pressure-test proceeded on:
   ...
   - **Different** → flag exactly what shifts — which acceptance criteria, and which `/iadc-advisor:to-spec` build
...
7. **Hand off when clear.** On `Status: READY`, tell the builder the ticket is ready and the next
   step is **`/iadc-advisor:to-spec`** (or, if a provisional spec already exists, that `/iadc-advisor:to-spec` can now finalize
   it). If items remain open, say which, and that they can re-run `/iadc-advisor:reconcile <TICKET-KEY>`
...
builder's explicit yes (a follow-up to the lead follows `/iadc-advisor:pressure-test`'s escalation gate).

## Relationship to the other skills

- **Follows `/iadc-advisor:pressure-test`:** `ticket → /iadc-advisor:pressure-test → (/iadc-advisor:reconcile) → /iadc-advisor:to-spec`. Pressure-test raises and escalates
  the gaps; reconcile closes them and flips the ticket to `READY`.
- Uses `/iadc-advisor:jira` (read the ticket / comments) and the Slack connector (`slack_read_thread`) to find
  replies; `/iadc-advisor:domain-modeling` to record resolved decisions.
- **Gates `/iadc-advisor:to-spec`:** `/iadc-advisor:to-spec` reads the `Status:` line — a final build-ready spec requires `READY`.
