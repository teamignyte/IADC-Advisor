...
1. **`resolve-library-id`** — find the Appian documentation library. Search for `appian` and pick the Appian Suite help entry (e.g. an id like `/websites/appian_suite_help_26_6`). Prefer the version closest to the **`Appian version`** in the ambient **Project configuration** (from `docs/agents/project.md`, after any **Personal overrides**).
