...
Tell the user setup is complete and which skills now read from these files. **If `iadc` is still outstanding, say so again here** — step 9 already reported it once, but this is the last thing the user reads, so repeat it rather than let it drop: name `/iadc-graph:setup` and that it's fine to run whenever, in its own session. They can edit `docs/agents/*.md` and `.mcp.json` (gitignored, where step 2's rules were accepted) directly later — re-run this skill only to switch trackers or re-point the plugin at a different Appian project.
