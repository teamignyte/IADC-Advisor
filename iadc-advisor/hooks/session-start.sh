...
if safe_project_file "$project_dir/docs/agents/advisor.md"; then
  printf '\n## Project configuration (from docs/agents/advisor.md — written by /iadc-advisor:setup)\n\n'
  cat "$project_dir/docs/agents/advisor.md"
else
  printf '\n## Project configuration\n\nNot configured yet — run /iadc-advisor:setup to wire the MCP servers and project values.\n'
fi

if safe_project_file "$project_dir/docs/agents/advisor.local.md"; then
  printf '\n## Personal overrides (from docs/agents/advisor.local.md — gitignored; these values OVERRIDE the project configuration above)\n\n'
  cat "$project_dir/docs/agents/advisor.local.md"
fi
