...
**Main flow — a ticket → the dialectic → a build spec:** `ticket → /pressure-test → (/reconcile) → /to-spec`. **`/pressure-test`** is the default way to start when you're handed a ticket: a pure-Socratic, one-question-at-a-time interview that **asks but doesn't answer** (you do the reasoning), is **orient-led** (grounds in the app first) and **adaptive** (scales to blast radius), captures the glossary and decisions as it goes, and **escalates architectural gaps to the project lead** (async, via Slack/Jira) rather than forcing a call you can't own. **`/reconcile <TICKET>`** closes that loop when the lead replies — folding the answer in and flipping the ticket `BLOCKED → READY`. Then **`/to-spec`** synthesizes a developer-ready **build spec** (PRD context + ordered Appian build steps), presented for review and written only on approval; it **replaces** splitting into subtickets, and produces only a *provisional* spec while the ticket is `BLOCKED`. This is the 99% path. (`/interrogate-with-docs` is a sibling of Pressure-test that *proposes* answers as it goes.)

**Greenfield flow — shape net-new work:** `/interrogate-with-docs` (or `/interrogate-me`) → `/to-tickets`, for work that isn't a defined build yet. The tickets are the handoff point to developers outside this bundle — where each becomes a starting point for `/pressure-test`. `/interrogating` is the shared interview primitive; `/domain-modeling` keeps the glossary and ADRs sharp underneath it. _(A defined ticket skips this flow — `ticket → /pressure-test → /to-spec` produces the build spec directly.)_

**On-ramp:** `/wayfinder` for huge, foggy efforts (charts a map of decision tickets, produces decisions/ADRs not deliverables).

**Understand the app (no build in view):** `/orient` — a cited briefing on what the app does and how, composing the graph, live Appian, the board, and the glossary+ADRs (the packaged form of the inspection skills). For a new developer or catching up on an unfamiliar area; `/pressure-test` is the counterpart when you *do* have a build in view.
...
**Support:** `/research` (background agent, cited primary-source answers), `/handoff` (bridge a long session into a fresh one), `/to-diagram` (render a flowchart / sequence / ERD / state diagram from context — the shared diagram primitive `/orient`, `/to-spec`, `/pressure-test`, and `/to-tickets` draw on; saves to `outputs/` and presents it as an artifact).
