...
**Main flow — a ticket → the dialectic → a build spec:** `ticket → /gumby → (/gumby-reconcile) → /pokey`. **`/gumby`** is the default way to start when you're handed a ticket: a pure-Socratic, one-question-at-a-time interview that **asks but doesn't answer** (you do the reasoning), is **orient-led** (grounds in the app first) and **adaptive** (scales to blast radius), captures the glossary and decisions as it goes, and **escalates architectural gaps to the project lead** (async, via Slack/Jira) rather than forcing a call you can't own. **`/gumby-reconcile <TICKET>`** closes that loop when the lead replies — folding the answer in and flipping the ticket `BLOCKED → READY`. Then **`/pokey`** synthesizes a developer-ready **build spec** (PRD context + ordered Appian build steps), presented for review and written only on approval; it **replaces** splitting into subtickets, and produces only a *provisional* spec while the ticket is `BLOCKED`. This is the 99% path. (`/interrogate-with-docs` is a sibling of Gumby that *proposes* answers as it goes.)
