...
Two situations sit outside the main flow. The line is simple: `/gumby` is for a **defined** piece of work; these are for work that isn't defined yet.
...
**This is the handoff point.** The tickets go to developers (or coding agents) *outside* this bundle to implement — where each one becomes a starting point for `/gumby`. (For a defined ticket, `/gumby → /pokey` supersedes the `/to-spec → /to-tickets` tail: Pokey produces the build spec directly instead of splitting into subtickets.)
...
- **`/orient`** — a single **cited briefing** on the app (or one area, or one object): the shape and its hubs, the data model as an ERD, the decisions behind it, and what's in flight. It composes the inspection skills below — the graph, the live environment, the board, and the glossary + ADRs — into the narrated answer to "how does our app work." It's the packaged form of `/iadc-graph` + `/appian`; reach for those raw when you know the exact edge you want, and for `/orient` when you want the synthesis. The boundary with `/gumby` is simple: **have a build in view → `/gumby`; just understanding → `/orient`.**

## Appian knowledge & inspection

The domain skills that let the architect answer questions and ground plans in the real Appian application — the steps `/gumby` orchestrates, and the sources `/orient` composes. All are **read-only** — inspect and advise, never mutate.
...
- **`/office`** — read **Microsoft 365** source-of-truth documents (SharePoint/OneDrive requirements, specs, process docs) and Teams/Outlook discussion, via the Microsoft 365 connector. Grounds planning in what the spec actually says — `/gumby` searches these by ticket key. **Read-only**: find and cite, never send, upload, or edit.
...
- **`/domain-modeling`** — sharpen the project's *domain* language: challenge a fuzzy term, resolve an overloaded word ("account" doing three jobs), record a hard-to-reverse decision as an ADR. It's the active discipline `/gumby` and `/interrogate-with-docs` drive to keep the project glossary (`outputs/CONTEXT.md`) clean. (Project glossary and decision records are written to the git-ignored `outputs/` workspace — see `/gumby`.)
